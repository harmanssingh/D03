...
def hypotenuse(leg1,leg2):
    leg1sq=leg1**2
    leg2sq=leg2**2
    hypotenuse=math.sqrt(leg1**2+leg2**2)           #Pythagoras Theorem
    print(hypotenuse)
...
def is_palindrome(inputvalue):
    inputvalue=inputvalue.lower()
    if(len(inputvalue)>1):          #single character is always a palindrome
        return (inputvalue[0]==inputvalue[-1] and is_palindrome(inputvalue[1:-1]))      #recursive function
    else:
        return True
...
def is_power(a,b):
    if(a<0 and a!=math.floor(a)):       #if a is not a positive integer, return False
        return False
    elif a==1:                          #base case: 1 is a power of every number since 0th power of every number is 1
        return True
    else:
        return(a%b==0 and is_power(a/b,b))      #recursive function
